...
//*/ freeme.chenming, 20171127. Fp Wake
import com.freeme.keyguard.TimeWasteLog;
//*/
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private void onFingerprintAuthenticated(int userId) {
        ...
        Trace.endSection();
        //*/ freeme.chenming, 20171127. Fp Wake
        if (TimeWasteLog.DEBUG_TIME_WASTE) {
            Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime onFingerprintAuthenticated: " + (System.currentTimeMillis() - beginTime));
        }
        //*/
    }
    ...
    private void handleFingerprintAuthFailed() {
        ...
        //*/ freeme.chenming, 20171127. Fp Wake
        if (TimeWasteLog.DEBUG_TIME_WASTE) {
            Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime handleFingerprintAuthFailed: " + (System.currentTimeMillis() - beginTime));
        }
        //*/
    }
    ...
    private FingerprintManager.AuthenticationCallback mAuthenticationCallback
        ...
        public void onAuthenticationFailed() {
            ...
            //*/ freeme.chenming, 20171127. Fp Wake
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime onAuthenticationFailed: " + (System.currentTimeMillis() - beginTime));
            }
            //*/
        };
        ...
        public void onAuthenticationSucceeded(AuthenticationResult result) {
            ...
            long beginTime = System.currentTimeMillis();
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                TimeWasteLog.markFingerSuccessTime();
            }
            //*/
            ...
            if ((result.getFingerprint() != null)) {
                mFingerId = result.getFingerprint().getFingerId();

                //  if the user hasn't use strong authentication (pattern, PIN, password) since a
                //  while and thus can't unlock with fingerprint, so do nothing, start app later
                //  in #onKeyguardVisibilityChanged()
                if (isUnlockingWithFingerprintAllowed()) {
                    com.freeme.hardware.fingerprint.FreemeFingerprintHelper.handleFingerprintAuthenticated(
                            mContext, mFingerId);
                }
            }
            //*/
            ...
            //*/ freeme.chenming, 20171127. Fp Wake
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime onAuthenticationSucceeded: " + (System.currentTimeMillis() - beginTime));
            }
            //*/
        }
        ...
        public void onAuthenticationAcquired(int acquireInfo) {
            ...
            long beginTime = System.currentTimeMillis();
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                TimeWasteLog.markFingerAcquireDownTime();
                TimeWasteLog.markWakeUpTime();
            }
            //*/
            handleFingerprintAcquired(acquireInfo);
            //*/ freeme.chenming, 20171127. Fp Wake
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime onAuthenticationAcquired: " + (System.currentTimeMillis() - beginTime));
            }
            //*/
        }
    };
...
/**    private void handleSimStateChange(int subId, int slotId, State state) {
    ...
    public void onKeyguardVisibilityChanged(boolean showing) {
        ...
        if (!showing) {
            //*/ freeme.chenming, 20170301. Fingerprint Custom 2.
            //  because of the user has use strong authentication (pattern, PIN, password), ready to restart App
            if (mFingerId != 0) {
                if(!mFingerprintAlreadyAuthenticated) {
                    com.freeme.hardware.fingerprint.FreemeFingerprintHelper.handleFingerprintAuthenticated(
                            mContext, mFingerId);
                }
                mFingerId = 0;
