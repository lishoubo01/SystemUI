...
import com.android.systemui.recents.views.CircleProgress;
...
public class RecentsActivity extends Activity implements ViewTreeObserver.OnPreDrawListener {
    ...
    private CircleProgress mCircelView;
    private TextView mTxMemory;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mClearLinearLayout = (LinearLayout) findViewById (R.id.clear_linearlayout);
        mCircelView = (CircleProgress) findViewById (R.id.clear_progress);
        mCircelView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    showCircelViewAnimate (false,mCircelView.getValue (),0);
        ...
        mTxMemory = (TextView) findViewById (R.id.tx_memory);
    ...
    private void reloadStackView() {
        ...
        updateClearLinearLayout();
        // After we have resumed, set the visible state until the next onStop() call
        mIsVisible = true;
    }

    private void updateClearLinearLayout() {
        initClearLinearLayout();
        MemInfoReader memReader = new MemInfoReader();
        memReader.readMemInfo();
        double realTotalRam = memReader.getTotalSize();
        int[] mMemStates = ProcessStats.ALL_MEM_ADJ;
        long now = SystemClock.uptimeMillis();
        long DURATION_QUANTUM = ProcessStats.COMMIT_PERIOD;
        long[] sDurations = new long[] {
        ...
        IProcessStats mProcessStats= IProcessStats.Stub.asInterface(ServiceManager.getService(ProcessStats.SERVICE_NAME));;
        ...
        ProcessStats mStats = new ProcessStats(false);
        try {
            ParcelFileDescriptor pfd = mProcessStats.getStatsOverTime(sDurations[0]);
        ...
        long memTotalTime = DumpUtils.dumpSingleTime(null, null, mStats.mMemFactorDurations, mStats.mMemFactor, mStats.mStartTime, now);
        ...
        android.util.Log.i("shouboli","updateClearLinearLayout()   usedRam:"+usedRam+" freeRam:"+freeRam+" totalScale:"+totalScale+" realFreeRam:"+realFreeRam);
        ActivityManager.MemoryInfo memInfo = new ActivityManager.MemoryInfo();
        ((ActivityManager) getApplicationContext ().getSystemService(Context.ACTIVITY_SERVICE)).getMemoryInfo(
                memInfo);
        ...
        int value = (int) ((realTotalRam-realFreeRam)*100/realTotalRam);
        String totalString = Formatter.formatShortFileSize(getApplicationContext (), (long) realTotalRam);
        String freeString = Formatter.formatShortFileSize(getApplicationContext (), (long)  realFreeRam);
        String values_memory = getString (R.string.tx_memory,freeString,totalString);
        mTxMemory.setText (values_memory);
        showCircelViewAnimate(true,0,value);
    }


    private void showCircelViewAnimate(boolean delay , int fromvalue ,int tovalue){
        mScrollAnimator= ObjectAnimator.ofInt (mCircelView, MEMORY, fromvalue, tovalue);
        mScrollAnimator.setDuration(600);
        mScrollAnimator.setInterpolator(Interpolators.LINEAR_OUT_SLOW_IN);
        if(delay){
            mScrollAnimator.setStartDelay (500);
        }
        mScrollAnimator.start ();
    }
    private void hideClearLinearLayout() {
        mClearLinearLayout.animate()
                .alpha(0f)
                .setDuration(100)
                ...
                .withEndAction(new Runnable() {
                    @Override
                    public void run() {
                        mClearLinearLayout.setVisibility(View.INVISIBLE);
                    }
                })
                .start();
    }

    private void initClearLinearLayout() {
        mClearLinearLayout.animate()
                .alpha(1f)
                .setDuration(100)
                .setInterpolator(Interpolators.FAST_OUT_SLOW_IN)
                .withEndAction(new Runnable() {
                    @Override
                    public void run() {
                        mClearLinearLayout.setVisibility(View.VISIBLE);
                    }
                })
                .start();
        mCircelView.setValue (0);
    }

    private static final Property<CircleProgress, Integer> MEMORY =
            new IntProperty<CircleProgress> ("memory"){
                @Override
                public void setValue(CircleProgress object, int value) {
                    object.setValue(value);
                }

                @Override
                public Integer get(CircleProgress object) {
