...
public class TaskStackLayoutAlgorithm {
    ...
    public void initialize(Rect displayRect, Rect windowRect, Rect taskStackBounds,
        ...
        /*/ freeme.gouzhouping, 20170619. show recents clear button.
        mStackActionButtonRect.set(mStackRect.left, mStackRect.top - topMargin,
                mStackRect.right, mStackRect.top + mFocusedTopPeekHeight);
        /*/
        mStackActionButtonRect.set(mStackRect.left, mStackRect.bottom - mFocusedBottomPeekHeight - mStackBottomOffset,
                mStackRect.right, mStackRect.bottom - mStackBottomOffset);
        //*/
        ...
        android.util.Log.i("lishoubo","TaskStackLayoutAlgorithm.java initialize() mTaskRect:"+mTaskRect.toString ());
    ...
    void update(TaskStack stack, ArraySet<Task.TaskKey> ignoreTasksSet) {
        ...
        if (getInitialFocusState() == STATE_FOCUSED) {
            ...
            // stack bottom offset
            int maxBottomOffset = mStackBottomOffset + mTaskRect.width ();
    ...
    public TaskViewTransform getStackTransform(Task task, float stackScroll, int focusState,
        ...
        if (mFreeformLayoutAlgorithm.isTransformAvailable(task, this)) {
            ...
            android.util.Log.i("lishoubo","TaskStackLayoutAlgorithm.java getStackTransform() Tittle:"+task.title);
    ...
    public void getStackTransform(float taskProgress, float nonOverrideTaskProgress,
        ...
        if (!forceUpdate && !unfocusedVisible && !focusedVisible) {
            transformOut.reset();
            android.util.Log.i("lishoubo","TaskStackLayoutAlgorithm.java getStackTransform() run to return");
            return;
        }
        ...
        if (!ssp.hasFreeformWorkspaceSupport() && mNumStackTasks == 1 && !ignoreSingleTaskCase) {
            ...
            int centerYOffset = (mStackRect.left - mTaskRect.left) +
                    (mStackRect.width () - mSystemInsets.right - mTaskRect.width ()) / 2;
            x = centerYOffset + getYForDeltaP(tmpP, 0);
            z = mMaxTranslationZ;
            dimAlpha = 0f;
            ...
            unfocusedY= (int) ((1f - mUnfocusedCurveInterpolator.getInterpolation(unfocusedRangeX)) * mStackRect.width ());
            ...
            android.util.Log.i("lishoubo","focusState:"+focusState+" unfocusedY:"+unfocusedY+" focusedY:"+focusedY+" x:"+x);
        }
...
//        android.util.Log.i("lishoubo","TaskStackLayoutAlgorithm.java getStackTransform() mTaskRect:"+mTaskRect.toString ()+" x:"+x+" Y:"+y+" taskProgress:"+taskProgress+" nonOverrideTaskProgress:"+nonOverrideTaskProgress+" \nstackScroll:"+stackScroll+" mStackRect.left - mTaskRect.left:"+(mStackRect.left - mTaskRect.left)+
//        " focusState:"+focusState+" unfocusedY:"+unfocusedY+" focusedY:"+focusedY);
    ...
    public float getDeltaPForY(int downY, int y) {
        float deltaP = (float) (y - downY) / mStackRect.width () *
    ...
    public int getYForDeltaP(float downScrollP, float p) {
        int y = (int) ((p - downScrollP) * mStackRect.width () *
    ...
    private float getNormalizedXFromUnfocusedY(float y, @AnchorSide int fromSide) {
        float offset = (fromSide == FROM_TOP)
                ? mStackRect.width () - y
                : y;
        float offsetPct = offset / mStackRect.width ();
