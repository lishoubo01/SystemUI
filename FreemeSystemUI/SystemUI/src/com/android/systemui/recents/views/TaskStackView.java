...
public class TaskStackView extends FrameLayout implements TaskStack.TaskStackCallbacks,
   ...
   public void updateLayoutAlgorithm(boolean boundScrollToNewMinMax) {
        ...
        event.addPostAnimationCallback(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = tasks.size() - 1; i >= 0; i--) {
                    ...
                    /*/
                    if (!((Task)tasks.get(i)).isTaskLocked(getContext())) {
                        EventBus.getDefault().send(new DeleteTaskDataEvent(tasks.get(i)));
                    }
                    //*/
                }
    ...
    public final void onBusEvent(TaskViewDismissedEvent event) {
        ...
        //*/ freeme.gouzhouping, 20170705. lock to app.
        if (event.task.key.baseIntent != null && event.task.isTaskLocked(mContext)) {
            LockTaskPreference.getInstance(mContext).removePreference(mContext,
                    event.task.key.baseIntent.getComponent().flattenToShortString());
        }
        //*/
