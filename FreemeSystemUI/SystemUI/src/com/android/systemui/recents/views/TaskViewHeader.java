...
public class TaskViewHeader extends FrameLayout
    ...
    //*/ freeme.gouzhouping, 20170705. lock to app.
    ...
    protected void onFinishInflate() {
        ...
        //*/ freeme.gouzhouping, 20170705. lock to app.
        mLockAppButton = (ImageView) findViewById(R.id.lock_app);
        mLockAppButton.setOnClickListener(this);
        //*/
    ...
    private void updateLayoutParams(View icon, View title, View secondaryButton, View button) {
        ...
        //*/ freeme.gouzhouping, 20170705. lock to app.
        int marginEnd = (int)getResources().getDimension(R.dimen.freeme_task_header_margin_end);
        //*/
        ...
        //*/ freeme.gouzhouping, 20170705. lock to app.
        if (mLockAppButton != null) {
            lp = new FrameLayout.LayoutParams(mHeaderBarHeight, mHeaderBarHeight, Gravity.END);
            lp.setMarginEnd(marginEnd);
            mLockAppButton.setLayoutParams(lp);
            mLockAppButton.setPadding(mHeaderButtonPadding, mHeaderButtonPadding,
                    mHeaderButtonPadding, mHeaderButtonPadding);
        }
        //*/
        lp = new FrameLayout.LayoutParams(mHeaderBarHeight, mHeaderBarHeight, Gravity.END);
    ...
    public void onTaskDataLoaded() {
        if (mTask.icon != null) {
            ...
            //*/ freeme.gouzhouping. 20170705. lock to app.
            setLockTaskBtn(mTask.isTaskLocked(getContext()), mTask.useLightOnPrimaryColor);
            //*/
        }
    }
    ...
    void resetNoUserInteractionState() {
        ...
        //*/ freeme.gouzhouping. 20170705. lock to app.
        if (mTask != null && mLockAppButton != null) {
            setLockTaskBtn(mTask.isTaskLocked(getContext()), mTask.useLightOnPrimaryColor);
        }
        //*/
    }
    ...
    public void onClick(View v) {
        ...
        //*/ freeme.gouzhouping, 20170705. lock to app.
        else if (v == mLockAppButton) {
            TaskView tv = (TaskView) Utilities.findParent(this, TaskView.class);
            if (tv.getTask().key.baseIntent != null) {
                String lockComponentName = tv.getTask().key.baseIntent.getComponent().flattenToShortString();
                if (tv.getTask().isTaskLocked(getContext())) {
                    setLockTaskBtn(false, tv.getTask().useLightOnPrimaryColor);
                    LockTaskPreference.getInstance(getContext()).removePreference(getContext(), lockComponentName);
                } else {
                    setLockTaskBtn(true, tv.getTask().useLightOnPrimaryColor);
                    LockTaskPreference.getInstance(getContext()).savePreference(getContext(), lockComponentName);
                }
            }
        }
        //*/
    ...
    //*/ freeme.gouzhouping, 20170705. lock to app.
    private void setLockTaskBtn(boolean isLock, boolean isUseLightOnPrimaryColor) {
        if (isLock) {
            mLockAppButton.setImageResource(R.drawable.freeme_lock_app);
        } else {
            mLockAppButton.setImageResource(R.drawable.freeme_unlock_app);
        }
        setLockTaskTint(isUseLightOnPrimaryColor);
    }

    private void setLockTaskTint(boolean isUseLightOnPrimaryColor) {
        if (isUseLightOnPrimaryColor) {
            mLockAppButton.setColorFilter(getResources().getColor(R.color.recents_task_bar_light_icon_color));
        } else {
            mLockAppButton.setColorFilter(getResources().getColor(R.color.recents_task_bar_dark_icon_color));
        }
        mLockAppButton.invalidate();
    }
    //*/
