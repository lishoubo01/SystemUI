...
public abstract class QSTile<TState extends State> {
    ...
    public static abstract class Icon {
        ...
        //*/ freeme.gouzhouping, 20170623. tile tint color
        ...
        public void tintColor(Context context, State state, Drawable d) {
            d.setColorFilter(context.getResources().getColor(
                    (state != null && state instanceof BooleanState && ((BooleanState) state).value)
    ...
    protected class AnimationIcon extends ResourceIcon {
        ...
        //*/ freeme.gouzhouping, 20170623. tile tint color.
        public void tintColor(Context context, State state, Drawable d) {
            if (d == null || !(state instanceof BooleanState)) {
                return;
            }

            if (!(d instanceof AnimationDrawable)) {
                super.tintColor(context, state, d);
                return;
            }

            int fromTint = context.getColor(R.color.freeme_tile_icon_off);
            int toTint = context.getColor(R.color.freeme_tile_icon_on);
            float fromR = (float) ((fromTint >> 16) & 0xff);
            float fromG = (float) ((fromTint >> 8) & 0xff);
            float fromB = (float) (fromTint & 0xff);
            float toR = (float) ((toTint >> 16) & 0xff);
            float toG = (float) ((toTint >> 8) & 0xff);
            float toB = (float) (toTint & 0xff);

            AnimationDrawable a = (AnimationDrawable) d;
            int numFrame = a.getNumberOfFrames();
            int steps = numFrame - 1;
            float stepR = (toR - fromR) / steps;
            float stepG = (toG - fromG) / steps;
            float stepB = (toB - fromB) / steps;
            for (int i = 0; i < numFrame; i++) {
                Drawable frame = a.getFrame(i).mutate();
                frame.setTint(
                        (Math.round(i * stepR + fromR) << 16) |
                                (Math.round(i * stepG + fromG) << 8) |
                                (Math.round(i * stepB + fromB))
                );
            }
        }
        //*/
    ...
    public static class State {
        ...
        //*/ freeme, chenming, 20170912. set custom tile tint color
        public int state;
        //*/
