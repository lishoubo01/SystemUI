...
public class FalsingManager implements SensorEventListener {
    ...
    private void sessionExitpoint(boolean force) {
        if (mSessionActive && (force || !shouldSessionBeActive())) {
            ...
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime SensorManager unregisterListener : "
    ...
    private void onSessionStart() {
        ...
        if (mHumanInteractionClassifier.isEnabled()) {
            ...
            //*/ freeme.chenming, 20171127. Fp Wake
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime onSessionStart CLASSIFIER_SENSORS: "
                        + (System.currentTimeMillis() - beginTime));
            }
            //*/
        }
        if (mDataCollector.isEnabledFull()) {
            ...
            //*/ freeme.chenming, 20171127. Fp Wake
            if (TimeWasteLog.DEBUG_TIME_WASTE) {
                Log.d(TimeWasteLog.FP_LOG_TAG, "wasteTime onSessionStart COLLECTOR_SENSORS: "
                        + (System.currentTimeMillis() - beginTime));
            }
            //*/
        }
