...
public class NavigationBarInflaterView extends FrameLayout implements TunerService.Tunable {
    ...
    protected String getDefaultLayout() {
        ...
        if (FreemeOption.Navigation.supports(FreemeOption.Navigation.FREEME_NAVIGATION_COLLAPSABLE)) {
            mNavigationBarStyle = Settings.System.getIntForUser(mContext.getContentResolver(),
                    FreemeSettings.System.FREEME_NAVIGATION_BAR_STYLE, 0, UserHandle.USER_CURRENT);
            mNavigationBarCollapsable = Settings.System.getIntForUser(
                    mContext.getContentResolver(), FreemeSettings.System.FREEME_NAVIGATION_BAR_COLLAPSABLE, 1, UserHandle.USER_CURRENT) != 0;
            return mContext.getString(computeLayoutType());
        }
        //*/
    ...
    protected View inflateButton(String buttonSpec, ViewGroup parent, boolean landscape,
        ...
        /// @}
        //*/ freeme.zhiwei.zhang, 20170911. Navigation.
        } else if (FREEME_COLLAPSABLE_BAR.equals(button)) {
            v = inflater.inflate(R.layout.freeme_collapsable_bar, parent, false);
            if (landscape && isSw600Dp()) {
                setupLandButton(v);
            }
        //*/
    ...
    //*/ freeme.zhiwei.zhang, 20170911. Navigation.
    public static final String FREEME_COLLAPSABLE_BAR = "freeme_collapsable_bar";
    private int mNavigationBarStyle = 0;
    private boolean mNavigationBarCollapsable = false;

    private ContentObserver mNavigationBarCollapsabledObserver = new ContentObserver(new Handler()) {
        @Override
        public void onChange(boolean selfChange) {
            boolean navCollapsable = Settings.System.getIntForUser(mContext.getContentResolver(),
                    FreemeSettings.System.FREEME_NAVIGATION_BAR_COLLAPSABLE, 0, UserHandle.USER_CURRENT) == 1;
            if (mNavigationBarCollapsable != navCollapsable) {
                mNavigationBarCollapsable = navCollapsable;
                clearViews();
                inflateLayout(mContext.getString(computeLayoutType()));
            }
        }
    };

    private ContentObserver mNavigationBarStyleObserver = new ContentObserver(new Handler()) {
        @Override
        public void onChange(boolean selfChange) {
            int type = Settings.System.getIntForUser(mContext.getContentResolver(),
                    FreemeSettings.System.FREEME_NAVIGATION_BAR_STYLE, 0, UserHandle.USER_CURRENT);
            if (mNavigationBarStyle != type) {
                mNavigationBarStyle = type;
                clearViews();
                inflateLayout(mContext.getString(computeLayoutType()));
            }
        }
    ...
    private void registerContentObserver() {
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(FreemeSettings.System.FREEME_NAVIGATION_BAR_STYLE),
                true, mNavigationBarStyleObserver, UserHandle.USER_ALL);
        mContext.getContentResolver().registerContentObserver(
                Settings.System.getUriFor(FreemeSettings.System.FREEME_NAVIGATION_BAR_COLLAPSABLE),
                true, mNavigationBarCollapsabledObserver, UserHandle.USER_ALL);
    }

    private void unRegisterContentObserver() {
        mContext.getContentResolver().unregisterContentObserver(mNavigationBarStyleObserver);
        mContext.getContentResolver().unregisterContentObserver(mNavigationBarCollapsabledObserver);
    }

    private int computeLayoutType() {
        if (mNavigationBarCollapsable) {
            return (mNavigationBarStyle == 1) ? R.string.config_navBarLayout_collapsable_inversion : R.string.config_navBarLayout_collapsable_normal;
        } else {
            return (mNavigationBarStyle == 1) ? R.string.config_navBarLayout_inversion : R.string.config_navBarLayout;
        }
    }
    //*/
}
