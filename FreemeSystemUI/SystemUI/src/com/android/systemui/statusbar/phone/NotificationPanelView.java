...
public class NotificationPanelView extends PanelView implements
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mStatusBar.updateCarrierVisibility(mKeyguardShowing || mStatusBarState == StatusBarState.SHADE_LOCKED ||
                mQsContainer.getCustomizer().isCustomizing() ||
    ...
    private boolean handleQsTouch(MotionEvent event) {
        ...
        /*/
        boolean slideStatusBarEnable = mKeyguardShowing ? isForibSlide() : false;
        if (!mQsExpandImmediate && mQsTracking && !slideStatusBarEnable) {
        //*/
                onQsTouch(event);
    ...
    private void setQsExpanded(boolean expanded) {
        //*/ freeme.gouzhouping, 20170908. forbidden slide status bar when keyguard showing.
        if (mKeyguardShowing && isForibSlide()) return;
        //*/
    ...
    public void setBarState(int statusBarState, boolean keyguardFadingAway,
        ...
        if (oldState == StatusBarState.KEYGUARD
            ...
            //*/ freeme.gouzhouping. 20170830. brightness layout
            // fix slide up animation when return from bouncer, bug redo:
            // 1. connect the phone with usb
            // 2. scroll the minimal header in lockscreen with bouncer
            // 3. scroll the unlock icon to enter the bouncer window
            // 4. press back button in navigation bar
            // Then, the a part of empty white of notification panel left on the top of screen.
            mQsContainer.animateContainerSlidingOut();
            /*/
    ...
    private void updateQsState() {
        ...
        /*/ freeme.gouzhouping, 20170908. remove the shadow on edit window.
        ...
        //*/ freeme.gouzhouping, 20171019. show carrier label.
        if (!FreemeOption.Navigation.supports(FreemeOption.Navigation.FREEME_NAVIGATION_COLLAPSABLE)) {
            mQsNavbarScrim.setVisibility(SystemServicesProxy.getInstance(mContext).hasSoftNavigationBar() ?
    ...
    private boolean shouldQuickSettingsIntercept(float x, float y, float yDiff) {
        ...
        //*/ freeme.gouzhouping, 20170908. forbidden slide status bar when keyguard showing.
        header = mKeyguardShowing ? mKeyguardStatusBar : mQsContainer;
        //*/
        ...
        if (mQsExpanded) {
            ...
            /*/ freeme.gouzhouping,20171116. fixbug#20986, optimize slide animation.
            return onHeader;
            /*/
            return mKeyguardShowing && isForibSlide() ? false : onHeader;
            //*/
        }
    }
    ...
    protected float getPeekHeight() {
        /*/ freeme.gouzhouping, 20171024. optimize animation of clicking statusbar.
        if (mNotificationStackScroller.getNotGoneChildCount() > 0) {
        ...
        return mNotificationStackScroller.getNotGoneChildCount() > 0 ?
                ...
                mStatusBar.getStatusBarHeight() / 2;
        //*/
    }
    ...
    protected void onClosingFinished() {
        ...
        //*/ freeme.gouzhouping, 20170731. fix bug: cannot pull down notification menu sometimes
        // test: 1. pull down notification menu in quick setting mode
        //       2. press edit, then edit window is showing
        //       3. PRESS home button
        // now, notification menu cannot be pull down.
        if (mQsContainer.isCustomizing()) {
            mQsContainer.getQsPanel().closeDetail();
        }
        //*/
    ...
    public boolean canCameraGestureBeLaunched(boolean keyguardIsShowing) {
        //*/ freeme.gouzhouping, 20171016. MTK's patch V1.99_P8_P9.
    ...
    //*/ freeme.gouzhouping, 20170908. forbidden slide status bar when keyguard showing.
    public boolean isForibSlide() {
        return Settings.System.getIntForUser(mContext.getContentResolver(),
                FreemeStatusbarStateToolKit.FORB_SLIDE_KEYGUARD_SWITCH, 0, UserHandle.USER_CURRENT) != 0;
    }
    //*/

    //*/ freeme.gouzhouping, 20170920. show carrier label.
    public void updateQSNavScrimVisibility(boolean isShown) {
        if (!SystemServicesProxy.getInstance(mContext).hasSoftNavigationBar()) return;
        mQsNavbarScrim.setVisibility(isShown ? View.GONE : View.VISIBLE);
    }
    //*/
}
